...
def parse_arguments():
    ...
    parser.add_argument(
        "--train", type=int, help="1 for adversarial training, 0 for evaluating"
    )
...
def run_standard_training():
    ...
    # execute training
    t0 = time.time()
    model = utils.standard_train(
        model, data_tr, criterion, optimizer, scheduler, device
    )
    train_time = time.time() - t0

    # move model to cpu and store it
    model.to("cpu")
    torch.save(model.state_dict(), "trained-models/simple-cnn")
...
def run_free_adv_training():
    ...
    # execute training
    t0 = time.time()
    model = defenses.free_adv_train(
        model, data_tr, criterion, optimizer, scheduler, consts.PGD_Linf_EPS, device
    )
    train_time = time.time() - t0

    # move model to cpu and store it
    model.to("cpu")
    torch.save(model.state_dict(), "trained-models/simple-cnn-free-adv-trained")
...
def run_evaluation():
    ...
    trained_models = {}
    mpaths = {
        "standard": "trained-models/simple-cnn",
    ...
    for mtype in trained_models:
        acc = utils.compute_accuracy(trained_models[mtype], loader_test, device)
