...
def compute_backdoor_success_rate(model, data_loader, device, mask, trigger, c_t):
    count_success = 0
    count_all = 0
    with torch.no_grad():
        for data in data_loader:
...
def run_whitebox_attack(attack, data_loader, targeted, device, n_classes=4):
    ...
    for data in data_loader:
        ...
        if targeted:
            y = (
                y + torch.randint(low=1, high=n_classes, size=(len(y),), device=device)
            ) % n_classes
...
def save_as_im(x, outpath):
    ...
    """
    im = Image.fromarray((x * 255.0).astype(np.uint8)).convert("RGB")
    im.save(outpath)
