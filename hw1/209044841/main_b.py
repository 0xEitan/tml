...
# init attacks
attacks = [PGDAttack(models[i], eps=8 / 255.0, early_stop=False) for i in range(3)]
...
for i in range(3):  # src
    ...
    for j in range(3):  # target
        sr = compute_attack_success(
            models[j], x_adv, y, consts.BATCH_SIZE, False, device
        )
...
for i in range(3):  # src
    ...
    for j in range(3):  # target
        sr = compute_attack_success(
            models[j], x_adv, y, consts.BATCH_SIZE, True, device
        )
        transfer_success[i, j] = sr
...
attack = PGDEnsembleAttack(models[1:], eps=8 / 255.0, early_stop=False)
# untargeted attacks
x_adv, y = run_whitebox_attack(attack, data_loader, False, device)
sr_untarg = compute_attack_success(
    models[0], x_adv, y, consts.BATCH_SIZE, False, device
)
# targeted attacks
